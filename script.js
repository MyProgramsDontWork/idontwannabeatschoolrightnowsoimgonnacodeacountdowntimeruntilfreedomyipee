...
if (hotkeyInput) {
    hotkeyInput.addEventListener('input', () => {
        localStorage.setItem('hotkey', hotkeyInput.textContent.trim());
    });
}
...
function addRow() {
  if (rows.length >= 12) {
    document.getElementById('addRowBtn').style.backgroundColor = '#ef476f';
    document.getElementById('addRowBtn').style.color = '#ffffff';
    document.getElementById('addRowBtn').textContent = 'Max 12 Periods';
    return;
  } else {
  ...
  rows.push([name, startTime, dash, endTime]);
}

function removeRow() {
  document.getElementById('addRowBtn').style.backgroundColor = '#ffffff';
  document.getElementById('addRowBtn').style.color = '#000000';
  document.getElementById('addRowBtn').textContent = 'Add Period';
  const grid = document.querySelector('.scheduleConfiguration');
  if (rows.length === 1) return;
  const lastRow = rows.pop();
  lastRow.forEach(el => grid.removeChild(el));
}
